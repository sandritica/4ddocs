...
| colorType | Integer | &#8594; | color de fuente del Listbox (por defecto) o color de fondo del Listbox |
...
El comando **LISTBOX SET ROW COLOR** define el color de una fila o una celda en el listbox de tipo array designado por los parámetros *object* y *\**.

Si pasa el parámetro opcional *\** indica que el parámetro *object* es un nombre de objeto (cadena). Si no se pasa este parámetro, indica que el parámetro *object* es una variable. En este caso, se pasa una referencia de variable en lugar de una cadena.

Puede designar un listbox o una columna del listbox en el parámetro *object*:

* Cuando el *object* designa un listbox, el comando se aplica a la fila.
* Cuando el *object* designa una columna del listbox, el comando se aplica a la celda localizada en la intersección de columna/fila.

En *row*, pase el número de la fila en la que desea aplicar el nuevo color.
...
En el parámetro *colorType*, pase las constantes `lk background color`  o `lk font color`  para indicar si desea aplicar el color para el fondo o para el color de fuente de la fila. Si omite este parámetro, el color se aplica para el color de fuente.
