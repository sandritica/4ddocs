...
| expanded | Boolean | &#8594; | Indica si la sublista será desplegada o contraída |
...
Se pasa el texto del nuevo elemento en *itemText*. A partir de 4D v16 R4, si el elemento está asociado a una acción estándar, se puede pasar la constante ak standard action title en *itemText* para utilizar automáticamente el nombre de la acción localizada. Para más información, consulte la sección *Acciones estándar*.

Pase el número de referencia del nuevo elemento en *itemRef*. Aunque calificamos este número de referencia de elemento como único, realmente puede pasar el valor que desea. Consulte la sección *Gestión de listas jerárquicas* para obtener más información sobre el parámetro *itemRef*.

Si quiere que el elemento incluya subelementos, pase un número de referencia de lista válido en el parámetro *sublista*. En este caso, también debe pasar el parámetro *expanded*. Pase **True** o **False** en este parámetro de manera que esta sublista se muestre desplegada o contraída respectivamente.
