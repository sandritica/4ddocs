...
| row | Integer | &#8594; | Numero de fila |
| colorType | Integer | &#8594; | Color de fuente del list box (predeterminado) o color de fondo del list box |
...
El comando **LISTBOX Get row color** devuelve el color de una fila o celda en el list box designado por los parámetros *object* y *\** como una cadena CSS.
...
Pasar el parámetro opcional *\** indica que el parámetro *object* es un nombre de objeto (cadena). Si no pasa este parámetro, indica que el parámetro *object* es una variable. En este caso, se pasa una referencia de variable en lugar de una cadena. Puede designar un list box o una columna de list box en el parámetro *objeco*:

* cuando *object* designa un list box, el comando devuelve el color de la fila.
* cuando *object* designa una columna de list box, el comando devuelve el color de la celda.

En *row*, pase el número de la fila cuyo color desea obtener.

**Nota:** el comando no tiene en cuenta ningún estado oculto/visible de las filas del list box.

En el parámetro *colorType*, puede pasar la constante `lk background color`  o `lk font color`  (tema "*Listbox*") para averiguar el color de fondo o fuente de la fila. Si omite este parámetro, se devuelve el color de la fuente.  
